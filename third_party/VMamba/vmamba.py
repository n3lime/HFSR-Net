...
try:
    ...
    # Keep this single-file module importable when Triton is unavailable.  The
    # decorated kernel is never selected in that case (see cross_scan_fn), but
    # Python still needs the decorator and annotations while defining it.
    class _TritonStub:
        @staticmethod
        def jit(fn):
            return fn
...
class SelectiveScanCuda(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, oflex=True, backend=None):
        ...
        if backend not in {"oflex", "mamba"}:
            raise RuntimeError(f"selective-scan CUDA backend is unavailable: {backend}")
        ctx.backend = backend
        if backend == "oflex":
...
def selective_scan_fn(
    ...
    cuda_backend_available = u.is_cuda and (
        (backend == "oflex" and WITH_SELECTIVESCAN_OFLEX)
        or (backend == "mamba" and WITH_SELECTIVESCAN_MAMBA)
        or (backend is None and (WITH_SELECTIVESCAN_OFLEX or WITH_SELECTIVESCAN_MAMBA))
    )
    fn = SelectiveScanCuda.apply if cuda_backend_available else selective_scan_torch
...
try:
    ...
    # fvcore is only used by the standalone VMamba FLOP-analysis helper.  It
    # must not prevent Ultralytics from constructing or training VSSBlock.
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if not channel_first:
            y = y.permute(0, 2, 3, 1).contiguous()
        # selective-scan may intentionally accumulate in FP32 (oflex=True),
        # while validation converts the surrounding model and LayerNorm
        # parameters to FP16.  Restore the activation dtype before LayerNorm.
        y = y.to(x.dtype)
        y = self.out_norm(y)
